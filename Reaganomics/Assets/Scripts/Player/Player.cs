...
public class Player : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        playerMovement = gameObject.GetComponent<PlayerMovement3D>();
        gameManager = GameObject.FindGameObjectWithTag("Game Manager").GetComponent<GameManager>();
        mainCam = gameManager.MainCam;
        float theAngle = Mathf.Round(transform.rotation.eulerAngles.y / 45f) * 45f % 180f;
        theAngle += playerMovement.rotationOffset;
        mainCam.transform.position = transform.position + (Quaternion.Euler(playerMovement.rotationPitch,theAngle, 0) * new Vector3(0,6.5f,-9.3f));
        mainCam.transform.rotation = Quaternion.Euler(31.54f + playerMovement.rotationPitch, theAngle, 0);
    }

    
    void Update()
    {
        playerMovement.enabled = !(character.inBattle);
        playerMovement.inPrompt = inPrompt;
    }
    ...
    void FixedUpdate ()
    {
        if (!gameManager.battle)
        {
            ...
            mainCam.transform.position = Vector3.Lerp(mainCam.transform.position, transform.position + (Quaternion.Euler(playerMovement.rotationPitch,theAngle, 0) * new Vector3(0,6.5f,-9.3f)), .05f);
            mainCam.transform.rotation = Quaternion.Lerp(mainCam.transform.rotation, Quaternion.Euler(31.54f + playerMovement.rotationPitch, theAngle, 0), .05f);
        }
